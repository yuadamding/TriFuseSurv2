...
EPOCH_GRADCAM_STRICT="${EPOCH_GRADCAM_STRICT:-1}"
...
echo "[optimal-search] epoch_gradcam=$EPOCH_GRADCAM every=$EPOCH_GRADCAM_EVERY split=$EPOCH_GRADCAM_SPLIT slices=$EPOCH_GRADCAM_MAX_SLICES teacher_force=$EPOCH_GRADCAM_TEACHER_FORCE_ALPHA strict=$EPOCH_GRADCAM_STRICT"
...
env \
  ...
  EPOCH_GRADCAM_STRICT="$EPOCH_GRADCAM_STRICT" \
  DRY_RUN="$DRY_RUN" \
