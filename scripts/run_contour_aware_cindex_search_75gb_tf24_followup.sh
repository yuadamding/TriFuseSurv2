...
run_trial() {
  ...
  for fold in "${FOLDS[@]}"; do
    if [[ "$SKIP_FINISHED" == "1" ]] && fold_has_complete_risks "$trial" "$fold"; then
      echo "[tf24-followup] trial=$trial fold=$fold -> existing completed risk exports found, skipping training"
      continue
    fi
    pending_folds+=("$fold")
  done
  ...
  if (( ${#pending_folds[@]} == 0 )); then
    echo "[tf24-followup] trial=$trial -> all requested folds already finished; scoring existing outputs"
