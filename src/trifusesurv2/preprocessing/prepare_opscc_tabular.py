...
from trifusesurv2.utils.clinical import DEFAULT_CLINICAL_COLS
...
def _relativize_path_columns(df: pd.DataFrame, *, start_dir: str) -> pd.DataFrame:
    path_cols = [
        "patient_dir",
        "out_dir",
        "rtstruct_path",
    ...
    for col in path_cols:
        if col not in df.columns:
            continue
        df[col] = df[col].map(
            lambda value: os.path.relpath(os.path.normpath(str(value)), os.path.normpath(start_dir))
            if pd.notna(value) and str(value).strip() and os.path.isabs(str(value))
            else value
        )
    return df
...
def main():
    ...
    merged_df = _relativize_path_columns(merged_df, start_dir=os.getcwd())
