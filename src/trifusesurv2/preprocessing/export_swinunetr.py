#!/usr/bin/env python3
"""
trifusesurv2.preprocessing.export_swinunetr
...
Example run
-----------
PYTHONPATH=TriFuseSurv2_package/src python3 -m trifusesurv2.preprocessing.export_swinunetr \
  --root OPSCC \
...
def _relpath_for_metadata(path: str, *, start_dir: str) -> str:
    if not path:
        return path
    try:
        return os.path.relpath(os.path.normpath(path), start=os.path.normpath(start_dir))
    except Exception:
        return path
...
def process_patient(
    ...
    patient_dir_rel = _relpath_for_metadata(patient_dir, start_dir=metadata_start_dir)
    out_dir_rel = _relpath_for_metadata(out_dir, start_dir=metadata_start_dir)
    ct_out_rel = _relpath_for_metadata(os.path.join(out_dir, "ct.nii.gz"), start_dir=metadata_start_dir)
    mask_union_out_rel = _relpath_for_metadata(os.path.join(out_dir, "mask_union.nii.gz"), start_dir=metadata_start_dir)
    mask_primary_out_rel = _relpath_for_metadata(os.path.join(out_dir, "mask_primary.nii.gz"), start_dir=metadata_start_dir)
    mask_nodal_out_rel = _relpath_for_metadata(os.path.join(out_dir, "mask_nodal.nii.gz"), start_dir=metadata_start_dir)

    meta: Dict[str, Any] = {
        "patient_id": pid,
        "patient_id_norm": pid_norm,
        "patient_dir": patient_dir_rel,
        "out_dir": out_dir_rel,
        "status": "ok",
        "error": "",
        ...
        "ct_out_path": ct_out_rel,
        "mask_union_out_path": mask_union_out_rel,
        "mask_primary_out_path": mask_primary_out_rel,
        "mask_nodal_out_path": mask_nodal_out_rel,
    ...
    try:
        ...
        meta["rtstruct_path"] = _relpath_for_metadata(rt_path, start_dir=metadata_start_dir)
        ...
        # 6) write outputs
        sitk.WriteImage(ct01, os.path.join(out_dir, "ct.nii.gz"), useCompression=True)
        ...
        sitk.WriteImage(mask_union, os.path.join(out_dir, "mask_union.nii.gz"), useCompression=True)
        sitk.WriteImage(mask_primary, os.path.join(out_dir, "mask_primary.nii.gz"), useCompression=True)
        sitk.WriteImage(mask_nodal, os.path.join(out_dir, "mask_nodal.nii.gz"), useCompression=True)
...
def main():
    ...
    for i, pdir in enumerate(patient_dirs, 1):
        ...
        meta = process_patient(
            patient_dir=pdir,
            out_root=out_root,
            metadata_start_dir=metadata_start_dir,
            surv_map=surv_map,
